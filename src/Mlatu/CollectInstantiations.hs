-- |
-- Module      : Mlatu.CollectInstantiations
...
import Mlatu.Infer (typeSize)
import Mlatu.Infer qualified as Infer
import Mlatu.Informer (Informer (..))
import Mlatu.Instantiate qualified as Instantiate
import Mlatu.Instantiated (Instantiated (Instantiated))
import Mlatu.Kind qualified as Kind
import Mlatu.Mangle qualified as Mangle
import Mlatu.Monad (K)
import Mlatu.Name (GeneralName (..), Qualified (..), Unqualified (..))
import Mlatu.Pretty qualified as Pretty
import Mlatu.Queue (Queue)
import Mlatu.Queue qualified as Queue
...
collectInstantiations dictionary0 = do
  ...
  where
    ...
    go q0 term = case term of
      ...
      Lambda typ name varType body origin -> do
        (body', q1) <- go q0 body
        return (Lambda typ name varType body' origin, q1)
      Match hint typ cases else_ origin -> do
        (cases', q1) <-
          foldrM
            ( \(Case name body caseOrigin) (bodies, q) -> do
                (body', q') <- go q body
        ...
        return (Match hint typ cases' else' origin, q2)
      New {} -> proceed
      NewClosure {} -> proceed
      ...
      Push {} -> proceed
      Word typ fixity (QualifiedName name) args origin -> do
        let types = case Dictionary.lookup (Instantiated name []) dictionary0 of
        ...
        case Dictionary.lookup (Instantiated name args) dictionary of
          ...
          Nothing -> case Dictionary.lookup (Instantiated name []) dictionary of
            ...
            Just (Entry.Word category merge origin parent signature mTerm) ->
              case mTerm of
                Just term -> do
                  ...
                  (term'', q'') <- go q' term'
                  let entry' =
                        Entry.Word category merge origin parent signature $
                          Just term''
                  processQueue q'' $
                    Dictionary.insert
            ...
            Just (Entry.Type origin params ctors) -> do
              ...
              typ <- Infer.dataType origin params ctors dictionary0
              TypeValue _ size <- typeSize dictionary0 $ foldl' (:@) typ args
...
instantiateTypes dictionary type0 q0 = go type0
  where
    go typ = do
      kind <- Infer.typeKind dictionary typ
      case kind of
        Kind.Value -> case typ of
          (:@) {} -> do
            instantiation <- collect [] typ
            return $ Queue.enqueue instantiation q0
          -- TODO: Forall.
          _terminal -> return q0
        _otherKind -> return q0
    collect args = \case
      a :@ b -> do
        -- go b
        collect (b : args) a
      TypeConstructor _ (Constructor name) -> do
        -- go a
        return (name, args)
      _nonConstructor -> error "non-constructor in type application (requires HKTs)"
