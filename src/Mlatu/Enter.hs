...
fragment f =
  ...
  foldlMx declareType (view Fragment.types f)
    ...
    >=> foldlMx
      declareWord
      ( filter (\def -> view Definition.category def == Category.Permission) $
    ...
    >=> foldlMx
      declareWord
      ( filter (\def -> Category.Permission /= view Definition.category def) $
          view Fragment.definitions f
      )
   ...
   in case Dictionary.lookup (Instantiated name []) dictionary of
        ...
        Just (Entry.Trait _origin traitSignature)
          ...
          | view Definition.category definition == Category.Instance ->
            do
              let qualifier = view qualifierName name
              resolvedSignature <-
                Resolve.run $
                  Resolve.signature
                    dictionary
...
defineWord dictionary definition = do
  ...
  checkpoint
  let resolvedSignature = view Definition.signature definition
