...
import Mlatu.Base.Name (ClosureIndex (..), GeneralName (..), LocalIndex (..), Qualified (..), Unqualified (..))
import Mlatu.Base.Origin (Origin)
import Mlatu.Base.Type (Constructor (..), Type (..), Var (..))
import Mlatu.Base.Type qualified as Type
import Mlatu.Base.Vocabulary
import Mlatu.Front.Definition (Parent (..))
...
import Mlatu.Front.Term (Term (..), Value (..), defaultElseBody)
...
inferType dictionary tenvFinal tenv0 term0 = case term0 of
  ...
  Group {} ->
    ice "group expression should not appear during type inference"
    ...
    let constructors = case cases of
          ...
          (_, QualifiedName ctorName, _) : _ ->
            case Dictionary.lookupWord (Instantiated ctorName []) dictionary of
              Just (Entry.WordEntry _ _ _ (Just (TypeParent typeName)) _ _) ->
                case Dictionary.lookupType (Instantiated typeName []) dictionary of
                  ...
                  _nonType -> ice "constructor not linked to type"
              _notFound -> ice "constructor not found after name resolution"
          _unqualified -> ice "nqualified constructor after name resolution"
...
inferCase
  ...
  (origin, qualified@(QualifiedName name), body) = do
    ...
    case Map.lookup name $ view TypeEnv.sigs tenv2 of
      ...
      Nothing ->
        ice "case constructor missing signature after name resolution"
inferCase _ _ _ _ _ = ice "case of non-qualified name after name resolution"
...
inferValue dictionary tenvFinal tenv0 origin = \case
  ...
  Quotation {} -> ice "quotation should not appear during type inference"
...
inferCall dictionary tenvFinal tenv0 (QualifiedName name) origin =
  case Map.lookup name $ view TypeEnv.sigs tenv0 of
    ...
    Just {} -> ice "what is a non-quantified type doing as a type signature?"
    Nothing -> do
...
typeKind dictionary = go
  where
    ...
    go t = case t of
      ...
      TypeValue {} -> ice "TODO: infer kind of type value"
