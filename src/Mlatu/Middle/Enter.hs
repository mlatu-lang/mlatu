...
import Mlatu.Base.Origin (point)
...
import Mlatu.Front.Definition (Category (..), Definition, Merge (..))
...
import Mlatu.Front.Parse qualified as Parse
...
import Mlatu.Middle.DesugarInfix qualified as Infix
import Mlatu.Middle.DesugarQuotations qualified as Quotations
...
import Mlatu.Middle.Resolve qualified as Resolve
import Mlatu.Middle.Scope (scope)
import Mlatu.Pretty (printQualified)
import Mlatu.TypeSystem.Infer (mangleInstance, typecheck)
...
fragment f =
  enterExtern
    ...
    >=> foldlMx
      declareWord
      ( filter ((== PermissionWord) . view Definition.category) $
   ...
   in case Dictionary.lookupWord (Instantiated name []) dictionary of
        ...
        _ -> case Dictionary.lookupTrait (Instantiated name []) dictionary of
          ...
          Just (Entry.TraitEntry _origin traitSignature)
            ...
            | view Definition.category definition == InstanceWord ->
              do
        ...
        do
          ...
          (flattenedBody, dictionary') <-
            Quotations.desugar
              ...
              $ quantifyTerm typ typecheckedBody
          let entry =
                Entry.WordEntry
                  (view Definition.category definition)
                  (view Definition.merge definition)
    ...
    _ -> case Dictionary.lookupWord (Instantiated name []) dictionary of
      Just (Entry.WordEntry category merge origin' parent signature' Nothing)
        | maybe True (resolvedSignature ==) signature' ->
          Quotations.desugar dictionary (qualifierFromName name) (quantifyTerm typ typecheckedBody)
            >>= \(flattenedBody, dictionary') ->
              pure $
                Dictionary.insertWord
                  (Instantiated name [])
      ...
      Just (Entry.WordEntry category merge@ComposeMerge origin' parent mSignature body)
        | view Definition.inferSignature definition
          ...
          (flattenedBody, dictionary') <-
            Quotations.desugar
              dictionary
              (qualifierFromName name)
              $ quantifyTerm composedType composed
          let entry =
                Entry.WordEntry
                  category
                  merge
      ...
      Just (Entry.WordEntry _ DenyMerge originalOrigin _ (Just _sig) _) -> do
...
quantifyTerm :: Type -> Term a -> Term a
quantifyTerm (Forall origin (Var name x Kind.Value) t) e = Term.Generic origin name x (quantifyTerm t e)
